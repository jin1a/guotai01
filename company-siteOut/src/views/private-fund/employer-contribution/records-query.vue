...
const handleViewDetail = (row) => {
  console.log('点击私退金详情按钮，行数据:', row)
  console.log('准备跳转到路径:', `/private-fund/employer-contribution/records-detail/${row.id}`)
  
  // 跳转到详情页面
  router.push(`/private-fund/employer-contribution/records-detail/${row.id}`)
    .then(() => {
      console.log('私退金路由跳转成功')
    })
    .catch(error => {
      console.error('私退金路由跳转失败:', error)
