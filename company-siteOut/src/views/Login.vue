        ...
        >
          ...
          <el-form-item prop="email">
            <el-input
              ...
              placeholder="請輸入電郵或計劃號（如：MPF-HK001-2024）"
              class="login-input"
              prefix-icon="Message"
            />
            <div class="input-hint">
              <el-text size="small" type="info">
                支持電郵地址或計劃號登錄（格式：MPF-xxx-yyyy 或 ORSO-xxx-yyyy）
              </el-text>
            </div>
          </el-form-item>
          ...
          <div class="register-section">
            <router-link to="/company-register?demo=true" class="register-link">計劃開通賬號 ></router-link>
          </div>
        </el-form>
        </div>
      </div>
...
import { isPlanCodeLogin, loginWithPlanCode } from '@/utils/mockData'
...
const handleLogin = async () => {
  ...
  try {
    const loginIdentifier = loginForm.email
    const password = loginForm.password
    
    // 检测是否为计划号登录
    if (isPlanCodeLogin(loginIdentifier)) {
      console.log('检测到计划号登录:', loginIdentifier)
      
      try {
        // 尝试计划号登录
        const planLoginResult = loginWithPlanCode(loginIdentifier, password)
        
        // 设置临时登录状态
        localStorage.setItem('token', planLoginResult.token)
        localStorage.setItem('currentPlanCode', loginIdentifier)
        userStore.$patch({
          token: planLoginResult.token,
          userInfo: planLoginResult.userInfo
        })
        
        ElMessage.success('計劃號登錄成功，建議升級您的賬戶以獲得更好體驗')
        
        // 跳转到账号升级页面
        setTimeout(() => {
          router.push(`/account-upgrade?planCode=${loginIdentifier}`)
        }, 1500)
        
        return
      } catch (error) {
        console.error('计划号登录失败:', error)
        ElMessage.error('計劃號或密碼錯誤')
        return
      }
    }
    ...
    console.log('普通登录，加密密码:', encryptedPassword)
    ...
    // 模拟普通登录成功，设置 token
    localStorage.setItem('token', 'normal-user-token')
    userStore.$patch({
    ...
    ElMessage.error('登錄失敗，請檢查您的登錄信息')
  }
}
</script>
